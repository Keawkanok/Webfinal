$(document).ready(function() {
    ...
    $(".comment-container").delegate(".reply", "click", function() {
        ...
        var form = '<form method="post" action="/repliesthrees"><input type="hidden" name="_token" value="' + token + '"><input type="hidden" name="comment3_id" value="' + cid + '"><input type="hidden" name="name" value="' + name + '"><div class="form-group"><textarea class="form-control" name="reply" placeholder="Enter your reply" > </textarea> </div> <div class="form-group"> <input class="btn btn-primary" type="submit"> </div></form>';
    ...
    $(".comment-container").delegate(".reply-to-reply", "click", function() {
        ...
        var token = $(this).attr("token")
        var form = '<form method="post" action="/repliesthrees"><input type="hidden" name="_token" value="' + token + '"><input type="hidden" name="comment3_id" value="' + cid + '"><input type="hidden" name="name" value="' + rname + '"><div class="form-group"><textarea class="form-control" name="reply" placeholder="Enter your reply" > </textarea> </div> <div class="form-group"> <input class="btn btn-primary" type="submit"> </div></form>';
    ...
    $(".comment-container").delegate(".delete-reply", "click", function() {
        ...
        // ถ้า delete response นี้ ถ้า comment คุณก็ไม่สามารถลบได้
        if (confirm("Are you sure you want to delete this..!")) {
            var did = $(this).attr("did");
            var token = $(this).attr("token");
            $.ajax({
                url: "/repliesthrees/" + did,
                method: "POST",
                data: { _method: "delete", _token: token },
                success: function(response) {
                    if (response == 1) {
